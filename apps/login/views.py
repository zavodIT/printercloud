...
def callback(request):
    ...
    # Decode the ID token using RS256 and the Auth0 public key
    try:
        jwks = get_auth0_jwks()
        ...
        if rsa_key:
            ...
            request.session['user'] = {
                ...
                'name': payload['name'],
                'email': payload['email'],
                'picture': payload.get('picture', '')  # Optional: Profile picture if available
            }
            return redirect('/')
    except jwt.JWTError as e:
        print(f"JWT decoding error: {e}")
        return redirect('/login')  # Handle the error appropriately
